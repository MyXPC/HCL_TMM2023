...
class MS_resnet_layer(nn.Module):
    ...
    def __init__(self, model, net_id, feature_size, classes_num):
        ...
        if net_id==18:
            ...
            BasicConv(256*3, feature_size, kernel_size=1, stride=1, padding=0, relu=True),  # 降维
    ...
    def forward(self, x, layer):
        ...
        """
        # 提取多尺度特征
        xf1, xf2, xf3, xf4, xf5 = self.features(x)  # 从ResNet提取5个层级的特征
        ...
        xl_part_contrast = self.conv_block_map(xl_part_contrast)  # 特征映射#(B,9,14,14)
        xl_concat = self.ada_maxpool1(xl_part_contrast.clone())  # 全局池化到1x1
        xl_concat = xl_concat.contiguous().view(xl_concat.size(0), -1)  # 展平 #(B,9)
        ...
        # 返回所有信息（用于训练和可视化）
        return xc1, xc2, xc3, x_concat, coord, xl_concat, xl_part_contrast, xf_ori #xc与x_concat为分类器得出结果，xl_concat为不同层提取的不同尺度的信息的特征向量
